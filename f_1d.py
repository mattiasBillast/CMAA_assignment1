...
def main():
    ...
    if test==1:
        ...
        n_grid = np.shape(grid)[0]
        V = np.zeros((1, n_grid))
        P = 0.1 + 0.0001 * np.exp(-(grid - 0.5) ** 2 / 0.1 ** 2)
        Rho = gamma * P
        t_max = 3
    s=(3,3)
    # A = np.zeros(s)
    ...
    while t<t_max:
        ...
        for i in xrange(1,n_grid-1):
            ...
            z = np.array([np.matmul(decom['L'],values[:,i,-2])]).T-dt/(2*dx)*np.multiply(np.array([decom['e']]).T,verschil)
            #print(decom['R'])
    ...
    values.tofile('test2x02t001.dat')
    plt.figure(1)
    plt.plot(values[0,:,-1])
    plt.ylabel('rho')
